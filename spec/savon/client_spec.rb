...
describe Savon::Client do
  ...
  describe "#request" do
    it "returns the request without making an actual call" do
      ...
      client = new_client(:endpoint => @server.url(:repeat))
      request = client.request(:authenticate) do
        message(:symbol => "AAPL" )
      end
    ...
    it "accepts a block with one argument" do
      ...
      request = client.request(:authenticate) do |locals|
        locals.message(@instance_variable)
      end
    ...
    it "accepts argument for the message tag" do
      client = new_client(:endpoint => @server.url(:repeat))
      request = client.request(:authenticate, :attributes => { "ID" => "ABC321" })
    ...
    it "raises when the operation name is not a symbol" do
      expect { new_client.request("not a symbol") }.to raise_error
    end

    it "raises when given an unknown option via the Hash syntax" do
      expect { new_client.request(:authenticate, :invalid_local_option => true) }.to raise_error
    end
